...
{
    public class RemoveAdminCommandHandler : ICommandHandler
    {
        private readonly IBraktApiClient _client;
        private readonly IContextFactory _contextFactory;
        private readonly Regex _mentionedUserRgx = new Regex(@"\<\@\!\d+\>");

        public RemoveAdminCommandHandler(IBraktApiClient client, IContextFactory contextFactory)
        {
            _client = client;
        ...
        public async Task ExecuteAsync(MessageCreateEventArgs args, CommandTokens cmdToken, IdContext userContext, CancellationToken cancellationToken)
        {
            ...
            {
                await args.Message.RespondAsync("Only admins can remove administrative priveleges from another player.");
                return;
            }
            ...
            foreach (var mention in cmdToken.Arguments.Where(w => _mentionedUserRgx.IsMatch(w)))
            {
                ...
                await _client.SetGroupAdminAsync(subjectContext.GroupMember.GroupId, subjectContext.GroupMember.PlayerId, cancellationToken, false);
            }
